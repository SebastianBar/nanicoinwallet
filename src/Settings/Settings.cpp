...
const char DEFAULT_WALLET_FILE_NAME[] = "nanicoin.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("nanicoinwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("nanicoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("nanicoinwallet.desktop");
  ...
  res = autorunSettings.contains("NaniCoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("NaniCoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.nanicoin.nanicoinwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=NaniCoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("NaniCoinWallet", appPath);
    } else {
      autorunSettings.remove("NaniCoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\nanicoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:nanicoin");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\nanicoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\nanicoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("nanicoinwallet.cfg"));
