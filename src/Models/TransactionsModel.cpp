...
QByteArray TransactionsModel::toCsv() const {
  QByteArray res;
  res.append("\"Estado\",\"Fecha\",\"Monto\",\"Comisión\",\"Hash\",\"Altura\",\"Transferencias\",\"ID de pago\"\n");
...
QVariant TransactionsModel::getDisplayRoleData(const QModelIndex &_index) const {
  ...
  case COLUMN_STATE: {
    ...
    QString transactionConfirmationState = transactionConfirmationCount < 1 ? tr("no confirmada") : tr("confirmada");
...
QVariant TransactionsModel::getToolTipRole(const QModelIndex& _index) const {
  if (_index.column() == COLUMN_AMOUNT && _index.data(ROLE_IS_FUSION_TRANSACTION).toBool()) {
    return tr("Transacciones de fusión te ayudan a optimizar tu monedero.");
  }
...
void TransactionsModel::showAllCachedTransactions() {
  ...
  m_lastVisibleTransactionIndex = m_transactions.size() - 1;
  endInsertRows();
  WalletLogger::debug(tr("[Transacciones] Transacciones insertadas: última=%1").arg(m_transactions.size() - 1));
}
