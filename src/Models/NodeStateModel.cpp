...
QVariant NodeStateModel::getDisplayRole(const QModelIndex& _index) const {
  ...
  case COLUMN_PEER_COUNT: {
    if (m_peerCount == 0) {
      ...
      return tr("%1 pares").arg(m_peerCount);
    }
  }
  ...
  case COLUMN_LAST_LOCAL_BLOCK_TIMESTAMP:
    ...
    return QLocale(QLocale::Spanish).toString(QDateTime::fromTime_t(m_lastLocalBlockInfo.timestamp).toUTC(), "dd MMM yyyy, HH:mm:ss");
