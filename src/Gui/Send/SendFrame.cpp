...
void SendFrame::urlReceived(const QUrl& _url) {
  ...
  qreal amount = QLocale(QLocale::Spanish).toDouble(m_cryptoNoteAdapter->formatUnsignedAmount(urlQuery.queryItemValue(PAYMENT_URL_AMOUNT_TAG).toULongLong()));
...
void SendFrame::sendClicked() {
  ...
  QuestionDialog dlg(tr("Precaución"), tr("Transacciones con un alto nivel de anonimidad pueden tomar varios minutos e incluso\nmás en ser generadas. "
    "Tales transacciones pueden ser muy grandes para ser aceptadas\nactualmente por la red.\nPor favor confirme que desea generar esta transacción."),
    m_mainWindow);
...
void SendFrame::setPaymentIdError(bool _error) {
  ...
  if (_error) {
    m_ui->m_paymentIdTextLabel->setText(tr("FORMATO INVÁLIDO"));
  } else{
    m_ui->m_paymentIdTextLabel->setText(tr("ID DE PAGO"));
  }
...
void SendFrame::setMixinError(bool _error) {
  ...
  if (_error) {
    m_ui->m_mixinTextLabel->setText(tr("MIXIN ES MUY GRANDE"));
  } else{
    m_ui->m_mixinTextLabel->setText(tr("NIVEL DE ANONIMIDAD"));
  }
