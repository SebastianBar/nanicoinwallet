...
void AddressBookFrame::deleteClicked(const QPersistentModelIndex& _index) {
  QString text(tr("Está seguro que deseas eliminar esta dirección?"));
  if (_index.data(AddressBookModel::ROLE_IS_DONATION_ADDRESS).toBool()) {
    text = tr("Esta dirección está siendo usada para donaciones. Si la eliminas la donación\n"
    "se apagará. Está seguro que desea eliminar esta dirección?");
  }

  QuestionDialog dlg(tr("Eliminar dirección"), text, m_mainWindow);
  if (dlg.exec() == QDialog::Accepted) {
...
void AddressBookFrame::contextMenu(const QPoint& _pos) {
  ...
  QAction* sendAction = new QAction(tr("Enviar"), &menu);
  QAction* editAction = new QAction(tr("Editar"), &menu);
  QAction* copyAction = new QAction(tr("Copiar al portapapeles"), &menu);
  QAction* delAction = new QAction(tr("Eliminar"), &menu);
