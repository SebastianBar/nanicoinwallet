...
void OptimizationOptionsFrame::initOptimizationPeriods() {
  m_ui->m_periodCombo->addItem(tr("30 minutos"), 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("1 hora"), HOUR_MSECS);
  m_ui->m_periodCombo->addItem(tr("1.5 horas"), HOUR_MSECS + 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("2 horas"), 2 * HOUR_MSECS);
  m_ui->m_periodCombo->addItem(tr("2.5 horas"), 2 * HOUR_MSECS + 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("3 horas"), 3 * HOUR_MSECS);
  m_ui->m_periodCombo->addItem(tr("3.5 horas"), 3 * HOUR_MSECS + 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("4 horas"), 4 * HOUR_MSECS);
  m_ui->m_periodCombo->addItem(tr("4.5 horas"), 4 * HOUR_MSECS + 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("5 horas"), 5 * HOUR_MSECS);
  m_ui->m_periodCombo->addItem(tr("5.5 horas"), 5 * HOUR_MSECS + 30 * MINUTE_MSECS);
  m_ui->m_periodCombo->addItem(tr("6 horas"), 6 * HOUR_MSECS);
}
...
void OptimizationOptionsFrame::updateEstimateValue() {
  ...
  if (walletAdapter->isOpen()) {
    ...
    if (estimate == 0) {
      ...
      m_ui->m_nonOptimizedOutputsTextLabel->setText(tr("Monedero está actualmente optimizado para este objetivo"));
    } else {
      m_ui->m_nonOptimizedOutputsLabel->show();
      m_ui->m_nonOptimizedOutputsLabel->setText(QString::number(estimate));
      m_ui->m_nonOptimizedOutputsTextLabel->setText(tr("Salida bajo el objetivo seleccionado"));
    }
  } else {
    m_ui->m_nonOptimizedOutputsLabel->hide();
    m_ui->m_nonOptimizedOutputsTextLabel->setText(tr("Monedero está cerrado"));
  }
}
