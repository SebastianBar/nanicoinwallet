...
MiningFrame::MiningFrame(QWidget* _parent) : QFrame(_parent), m_ui(new Ui::MiningFrame), m_cryptoNoteAdapter(nullptr),
  ...
  m_restorePoolListButton->setText(tr("Restaurar listado de pool por defecto"));
...
void MiningFrame::miningStarted() {
  ...
  if (!m_ui->m_startMiningButton->isChecked()) {
    ...
    m_ui->m_startMiningButton->setText(tr("Detener"));
...
void MiningFrame::miningStopped() {
  ...
  if (m_ui->m_startMiningButton->isChecked()) {
    ...
    m_ui->m_startMiningButton->setText(tr("Iniciar"));
...
void MiningFrame::deleteClicked(const QModelIndex& _index) {
  ...
  if (_index.data(MinerModel::ROLE_STATE).toInt() == IPoolMiner::STATE_RUNNING) {
    QMessageBox::warning(m_mainWindow, tr("Advertencia"), tr("No se puede eliminar el pool activo."));
    return;
  }

  QuestionDialog dlg(tr("Eliminar pool"), tr("El pool será eliminado de la lista. ¿Está seguro?"), m_mainWindow);
