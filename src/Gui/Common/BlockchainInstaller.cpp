...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_nanicoinDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfNaniCoinBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 nanicoinHeight;
  if (!checkIfBlockchainOutdated(currentHeight, nanicoinHeight)) {
    return;
  }

  QString questionStringTemplate = tr("Desea reemplazar su blockchain actual (altura: %1)\npor el que está en la carpeta de su monedero (altura: %2)?");

  QuestionDialog dlg(tr("Instalación de Blockchain"), QString(questionStringTemplate).arg(nanicoinHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfNaniCoinBlockchainExists() const {
  return m_nanicoinDir.exists() && m_nanicoinDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_nanicoinDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::nanicoinBlockchainInfo() const {
  return QFileInfo(m_nanicoinDir.absoluteFilePath(m_blocksFileName));
}

void BlockchainInstaller::copyProgress(quint64 _copied, quint64 _total) {
  Q_EMIT showMessageSignal(QString(tr("Copiando archivos del blockchain... (%1%)")).arg((qreal)_copied / _total * 100, 0, 'f', 0));
}

void BlockchainInstaller::installBlockchain() {
  Q_EMIT showMessageSignal(tr("Copiando archivos del blockchain..."));
  m_nanicoinDir.mkpath(".");
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_nanicoinDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_nanicoinDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
