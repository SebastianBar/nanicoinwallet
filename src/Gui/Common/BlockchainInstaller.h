...
private:
  ...
  const QDir m_nanicoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfNaniCoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _nanicoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo nanicoinBlockchainInfo() const;
