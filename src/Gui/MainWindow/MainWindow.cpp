...
MainWindow::MainWindow(ICryptoNoteAdapter* _cryptoNoteAdapter, IAddressBookManager* _addressBookManager,
  ...
  m_ui->setupUi(this);
  setWindowTitle(tr("Monedero NaniCoin %1").arg(Settings::instance().getVersion()));
...
void MainWindow::walletStateModelDataChanged(const QModelIndex& _topLeft, const QModelIndex& _bottomRight, const QVector<int>& _roles) {
  if (_topLeft.column() == WalletStateModel::COLUMN_ABOUT_TO_BE_SYNCHRONIZED) {
    ...
    if (!walletAboutToBeSynchronized) {
      ...
      m_ui->m_balanceLabel->installEventFilter(this);
      m_ui->m_balanceLabel->setToolTip(tr("Clic para copiar"));
    }
  }
}
...
void MainWindow::createWallet() {
  QString filePath = QFileDialog::getSaveFileName(this, tr("Nuevo archivo de monedero"),
#ifdef Q_OS_WIN
  ...
  if (QFile::exists(filePath)) {
    QMessageBox::warning(this, tr("Peligro"),
      tr("No puede sobreescribir el archivo existente %1 porque puede llevar a la pérdida de llaves privadas").arg(QFileInfo(filePath).fileName()));
    return;
  }
...
void MainWindow::openWallet() {
  QString filePath = QFileDialog::getOpenFileName(this, tr("Abrir archivo .wallet/.keys"),
#ifdef Q_OS_WIN
    QApplication::applicationDirPath(),
#else
    QDir::homePath(),
#endif
    tr("Monedero (*.wallet *.keys)"));
    ...
    if (filePath.endsWith(".keys")) {
      ...
      if (QFile::exists(walletFilePath)) {
        QMessageBox::warning(this, tr("Warning"),
          tr("No puede sobreescribir el archivo existente %1 porque puede llevar a la pérdida de llaves privadas").arg(QFileInfo(walletFilePath).fileName()));
        return;
      }
    } else {
      filePath.clear();
...
void MainWindow::backupWallet() {
  QString filePath = QFileDialog::getSaveFileName(this, tr("Respaldar monedero en..."),
#ifdef Q_OS_WIN
    QApplication::applicationDirPath(),
#else
    QDir::homePath(),
...
void MainWindow::saveWalletKeys() {
  QString filePath = QFileDialog::getSaveFileName(this, tr("Guardar llaves en..."),
#ifdef Q_OS_WIN
    QApplication::applicationDirPath(),
#else
    QDir::homePath(),
...
void MainWindow::resetWallet() {
  QuestionDialog dlg(tr("Reiniciar monedero?"), tr("Reiniciar monedero para resincronizar sus transacciones y saldo basado\n"
    "en los datos del blockchain. Esta operación puede tardar algún tiempo.\n"
    "Está seguro que desea reiniciar el monedero?"), this);
...
void MainWindow::removePendingTx() {
	...
	if (walletAdapter->resetPendingTransactions())
		QMessageBox::information(this, "Reiniciar transacciones", "Transacciones reiniciadas exitosamente.", QMessageBox::Ok);
	else
		QMessageBox::warning(this, "Error al reiniciar transacciones", "El reinicio de transacciones falló. Revise el registro de depuración.", QMessageBox::Ok);
}
...
void MainWindow::importKey() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (key.size() != sizeof(CryptoNote::AccountKeys)) {
      QMessageBox::warning(this, tr("Advertencia"), tr("Tamaño de llave de seguimiento incorrecto"));
      return;
    }

    QString filePath = QFileDialog::getSaveFileName(this, tr("Guardar seguimiento de monedero en..."),
#ifdef Q_OS_WIN
    QApplication::applicationDirPath(),
#else
    QDir::homePath(),
