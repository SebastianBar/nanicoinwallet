...
QString formatTimeDiff(quint64 _timeDiff) {
  ...
  QString secondPart;
   ...
   if (day > 0) {
    firstPart = QStringLiteral("%1 %2").arg(day).arg(day == 1 ? QObject::tr("día") : QObject::tr("días"));
    secondPart = QStringLiteral("%1 %2").arg(hour).arg(hour == 1 ? QObject::tr("hora") : QObject::tr("horas"));
  } else if (hour > 0) {
    firstPart = QStringLiteral("%1 %2").arg(hour).arg(hour == 1 ? QObject::tr("hora") : QObject::tr("horas"));
    secondPart = QStringLiteral("%1 %2").arg(minute).arg(minute == 1 ? QObject::tr("minuto") : QObject::tr("minutos"));
  } else if (minute > 0) {
    firstPart = QStringLiteral("%1 %2").arg(minute).arg(minute == 1 ? QObject::tr("minuto") : QObject::tr("minutos"));
  } else {
    firstPart = QStringLiteral("Menos de 1 minuto");
  }
...
void WalletStatusBar::walletOpened() {
  showMessage(tr("Monedero abierto"), TEMP_MESSAGE_DURATION);
...
void WalletStatusBar::walletClosed() {
  ...
  showMessage(tr("Monedero cerrado"));
}

void WalletStatusBar::passwordChanged() {
  showMessage(tr("Contraseña cambiada"), TEMP_MESSAGE_DURATION);
  updateEncryptedState(m_cryptoNoteAdapter->getNodeAdapter()->getWalletAdapter()->isEncrypted());
}
...
void WalletStatusBar::synchronizationProgressUpdated(quint32 _current, quint32 _total) {
  ...
  QString formattedTimeDiff = lastBlockTimestamp > 0 ? formatTimeDiff(timeDiff) : tr("desconocido");
  QString blockchainAge = lastBlockTimestamp > 0 ? QStringLiteral("%1 atrás").arg(formattedTimeDiff) : QStringLiteral("%1").arg(formattedTimeDiff);

  m_walletIsSynchronized = false;
  m_syncStatusLabel->setText(tr("Sincronización: %1/%2 (%3)").arg(_current).arg(_total).arg(blockchainAge));
