...
void NoWalletFrame::setPasswordError(bool _isErrorOccured) {
  ...
  m_ui->m_passwordTextLabel->setText(_isErrorOccured ? tr("CONTRASEÑA INCORRECTA") : tr("CONTRASEÑA PARA "));
...
void NoWalletFrame::okClicked() {
	...
	if (peer_count >= 1) {
		...
		//update ui
		m_ui->m_passwordTextLabel->setText(tr("ESPERANDO CONEXIONES ANTES DE ABRIR MONEDERO..."));
