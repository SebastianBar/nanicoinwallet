...
void TransactionsFrame::exportToCsv() {
  ...
  QString file = QFileDialog::getSaveFileName(m_mainWindow, tr("Seleccionar archivo CSV"), QDir::homePath(), "CSV (*.csv)");
  if (!file.isEmpty()) {
...
void TransactionsFrame::showFilter(bool _on) {
  m_ui->m_filterButton->setText(_on ? tr("Ocultar filtro") : tr("Mostrar filtro"));
