...
void OverviewHeaderFrame::miningStarted() {
  if (!m_ui->m_startMiningButton->isChecked()) {
    ...
    m_ui->m_miningStateLabel->setText(tr("Minando..."));
...
void OverviewHeaderFrame::miningStopped() {
  if (m_ui->m_startMiningButton->isChecked()) {
    ...
    m_ui->m_startMiningButton->setText(tr("Iniciar"));
    m_ui->m_miningStateLabel->setText(tr("No está minando"));
...
void OverviewHeaderFrame::transactionPoolChanged() {
  m_ui->m_overviewPoolSizeLabel->setText(tr("%1 transacciones").arg(m_transactionPoolModel->rowCount()));
}
