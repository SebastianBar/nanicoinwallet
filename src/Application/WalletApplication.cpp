...
const char BYTECOIN_URI_SCHEME_NAME[] = "nanicoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("nanicoinwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("nanicoinwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] NaniCoin wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Error"), "Monedero NaniCoin ya está en ejecución");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
      m_splash->showMessage(QObject::tr("Cargando blockchain..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      okButton->setText(QObject::tr("Aceptar"));
      dlg.addButton(okButton, QMessageBox::AcceptRole);
      dlg.setText(QObject::tr("La base de datos está siendo utilizada por otra aplicación o servicio.\n"
      "Si estás ejecutando nanicoind con un puerto RPC alternativo, debes cerrarlo y relanzar el monedero\n"
      "o\n"
      "Configurar el Daemon local con el puerto correcto en el monedero Menu/Preferencias/Conexión."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
    ...
    case INodeAdapter::INIT_DB_FAILED: {
      if (askForRemoveDb()) {
        ...
        if (!rmDir(dbPath)) {
          ...
          okButton->setText(QObject::tr("Aceptar"));
          dlg.addButton(okButton, QMessageBox::AcceptRole);
          dlg.setText(QObject::tr("Ocurrió un error"));
		  if (m_splash != nullptr)
			  m_splash->hide();
          dlg.exec();
          return false;
...
void WalletApplication::initUi() {
  ...
  if (m_splash != nullptr) {
    m_splash->showMessage(QObject::tr("Inicializando GUI..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
  }
...
void WalletApplication::showCoreInitError() {
  ...
  okButton->setText(QObject::tr("Aceptar"));
  dlg.addButton(okButton, QMessageBox::AcceptRole);
  dlg.setText(QObject::tr("Error al cargar el blockchain"));
  if (m_splash != nullptr)
	  m_splash->hide();
  dlg.exec();
}
...
{
  QuestionDialog dlg(tr("Error al cargar el blockchain"),
    tr("Borrar la carpeta de blockchain y sincronizar de cero?\n"
    "Importante! Si tienes archivos importantes en la misma carpeta, pueden ser\n"
    "borrados por igual. Estás seguro que deseas eliminar la carpeta y sincronizar?"), m_mainWindow);
  if (m_splash != nullptr)
	  m_splash->hide();
...
void WalletApplication::newLogString(const QString& _string) {
  ...
  if (match.hasMatch()) {
    QString message = _string.mid(match.capturedEnd()).prepend(tr("Importar"));
    if (m_splash != nullptr) {
