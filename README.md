...
```
git clone https://github.com/SebastianBar/nanicoinwallet.git
```
...
```
cp src/nanicoinwallet.desktop build/
cp src/images/nanicoin.png build/
cd build
linuxdeployqt.AppImage nanicoinwallet.desktop -appimage -verbose=2 -always-overwrite -no-translations
```
